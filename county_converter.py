...
with open('/Users/derekwillis/Downloads/Democrat.csv', 'rb') as csvfile:
    ...
    for county in COUNTIES:
        results = []
        filename = "20140506__oh__primary__democratic__%s.csv" % county.lower()
        ...
        for row in rows:
            ...
            for idx, cand in enumerate(cands):
                ...
                if 'District' in office:
                    ...
                    district = None
                votes = row[idx+5]
                results.append([county, row[1], row[2], row[3], row[4], office, district, 'D', cand.replace(' (D)', '').replace('  ', ' '), votes])
